/**
 * Commander adapter for the `metacode` command line.
 *
 ...
 * `@metacode/cmdline`). Launcher flags therefore come first: the first
 * token this parser does not recognize starts the inner arguments, so
 * `metacode --profile tui --resume abc` boots the tui profile with `--resume abc`,
 * and `metacode --profile web -h` prints the web app's help, not this one's.
 *
 * `web` is a hardcoded alias for `--profile web`; `plugin` manages a profile's
 * plugin dependencies by forwarding to pnpm.
 * @module @metacode/cli/args
 */
...
/** The resolved `metacode` invocation. Help, version, and errors exit inside {@link parseMetacodeArgs}. */
export type MetacodeInvocation = ProfileInvocation | DumpConfigInvocation | PluginInvocation
...
const HELP_EXAMPLES = `
Examples:
  metacode --profile web                          boot the web profile (same as: metacode web)
  metacode --profile headless "run the tests"     answer one task, print the result, and exit
  metacode --profile tui --patch ./extra.yml      boot a custom profile with one extra overlay
  metacode --profile tui --resume <session>       arguments after the launcher flags reach the app
  metacode --profile web --help                   the web app's own flags and help
  metacode plugin --profile tui add <package>     install a plugin into the tui profile
`
 ...
 */
function resolveBoot(program: Command, profile: string, options: BootOptions, args: string[]): MetacodeInvocation {
 ...
 * @returns the resolved invocation.
 */
export function parseMetacodeArgs(argv: readonly string[], version: string): MetacodeInvocation {
  let resolved: MetacodeInvocation | undefined
  ...
  program
    ...
    .description('metacode: boot a MetaCode Harness profile — an ordered stack of plugin-bundle patch layers under your own overrides.')
    .addHelpText('after', HELP_EXAMPLES)
    .exitOverride()
    // The launcher's flags come first and end at the first token it does not
    // know; everything from there on belongs to the booted app, including
    // its -h. `metacode -h` with no profile still prints this help, below.
    .helpOption(false)
    .allowUnknownOption()
    .passThroughOptions()
    .enablePositionalOptions()
    .argument('[args...]', 'arguments for the booted profile\'s app (see: metacode --profile <name> --help)')
    .option('--profile <name>', 'the profile under $METACODE_HOME/profiles to boot')
    .option('--patch <path>', 'extra patch-list overlay applied after the profile layer (repeatable)', collect)
    .option('--dump-config', 'print the composed profile tree and exit')
    .option('--dump-default-config', 'print the profile tree without its user layer or --patch overlays and exit')
    .action((args: string[], options: BootOptions & { profile?: string }) => {
      // With the app owning -h, the launcher's own help is what a bare
      // `metacode -h` (no profile to hand it to) must print.
  ...
  web
    ...
    .passThroughOptions()
    .enablePositionalOptions()
    .argument('[args...]', 'arguments for the web app (see: metacode web --help)')
    .option('--patch <path>', 'extra patch-list overlay applied after the profile layer (repeatable)', collect)
  ...
  if (resolved === undefined) throw new Error('metacode: no invocation resolved')
  return resolved
}

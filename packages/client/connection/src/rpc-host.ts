...
export class HostConnectionService extends Service implements HostConnectionHandle {
  ...
  requestRejection(request: ConnectionTrustRequest): ConnectionRequestRejection {
    ...
    return this.mobilePairingAuthenticated(request) ? undefined : 401
  }

  private mobilePairingAuthenticated(request: ConnectionTrustRequest): boolean {
    const token = parseBearerToken(request.headers)
      ?? parseAccessTokenFromUrl(requestUrl(request))
    if (token === undefined) return false
    const pairing = this.ctx.get('mobilePairing') as MobileSessionValidator | undefined
    return pairing !== undefined && pairing.validateSessionToken(token) !== undefined
  }
...
function requestUrl(request: ConnectionTrustRequest): string {
  const url = (request as ConnectionTrustRequest & { url?: unknown }).url
